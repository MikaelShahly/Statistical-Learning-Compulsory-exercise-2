...
HistGradientBoostedClassifier model:
  Best hyperparameters: 
    'max_depth': 4
    'learning_rate': 0.06526387443707683
    'max_iter': 143
    'max_features': 0.5
    'l2_regularization': 0.0035912380966771833
  
  Test accuracy:
    Tuned hyperparameters: 0.885869652173914
...
For the hyperparameter optimization we can see that we managed to improve it for the most accurat model, which is the most important. However the tuning with optuna introduced a heavy computational cost, as it was by far the optuna chunks that required the most time to run. Determining if it was worth it or not all depends on how highly you value accuracy. In the case of predicting heart disease the costs of a false negative is so high that it is probably worth spending a lot of time and resources to improve the model even slight amounts. So if this was supposed to be used by doctors we could have spent even more time on fine tuning the hyperparameters.

On the note of cost of failure we can argue that, given that the models are supposed to be used by doctors for treatnment of pasients, we should try to make a oversensitive model. That means that we should probably over categorize the heart disease population even though it hurts our accuracy, because the cost of a false negative is much higher than the cost of a false positive. We would of course much rather overtreat than undertreat when undertreatment can lead to death. That holds true until we have a capacity problem, which would cause the cost of a false positive to rise as we near the hospitals capacity. We could solve this by making a different models depending on if capacity is a problem or not, or we could maybe change what we are trying to predict from heart disease to death based of heart disease. That should allow us to add in a predictor value to represnt a capacity penalty, although this seems to be outside of the scope of the course.

Another aspect to note is that our we are try to predict if a person will get heart disease in head of time, because thats what is valuable, and not whether they have it right now or not. And to do this we are using data which only include if people have heart disease at the moment me observ them or not. This mean that we exclude some of the people who's predictors don't change but they will develop heart disease in the near future. And those people are closer to what we want to predict than who we use as positives right now. Thus our model is trained on something else than what we actually want to predict, and we have introduced a temporal bias into our models.
